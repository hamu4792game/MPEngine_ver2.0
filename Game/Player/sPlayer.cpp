...
WorldTransform sPlayer::PostUpdate() {
	// カメラアップデート
...
bool sPlayer::OnCollision(const AABB* aabb) {
	// ローカル変数にrつけてるだけ
...
void sPlayer::OnCollisionStage(const AABB* aabb) {
	// ローカル変数にrつけてるだけ
	bool iscoll = collision_->IsCollision(aabb);
	// 床との衝突判定
	if (iscoll) {
...
void sPlayer::DrawImGui() {
#ifdef _DEBUG
	ImGui::Begin("sPlayer",nullptr,ImGuiWindowFlags_MenuBar);
...
void sPlayer::BehaviorRootUpdate() {
	auto input = Input::GetInstance();
...
void sPlayer::BehaviorAttackUpdate() {
	// 予備動作の時間
	auto input = Input::GetInstance();
...
void sPlayer::BehaviorDashUpdate() {

}

void sPlayer::DoWireMoving() {
	static Vector3 rVec;
