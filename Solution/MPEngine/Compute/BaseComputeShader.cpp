...
void BaseComputeShader::Initialize(Model* model) {
	...
	uavDesc.Buffer.NumElements = model->GetModel()->GetModel().vertices.size(); // 頂点数
	uavDesc.Buffer.CounterOffsetInBytes = 0;
	uavDesc.Buffer.Flags = D3D12_BUFFER_UAV_FLAG_NONE;
	uavDesc.Buffer.StructureByteStride = sizeof(VertexData);

	auto device = DeviceManager::GetInstance()->GetDevice();
	device->CreateUnorderedAccessView(outputVertexResource_.Get(), nullptr, &uavDesc, outputVertexHandle_.get()->GetCPU());
...
void BaseComputeShader::UpdateProcess(Model* model) {
	...
	list->SetComputeRootDescriptorTable(0, model->GetAnimation()->skinCluster_.influenceResource->GetGPUVirtualAddress());
...
void BaseComputeShader::CreatePipeline() {
